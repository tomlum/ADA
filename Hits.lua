...
function drawhexcheck(ex, why, w, h, v, j)
  ...
  love.graphics.line(t[oppn].x,t[oppn].y, d[disn].x,d[disn].y)
  love.graphics.line(t[adjn1].x,t[adjn1].y, d[adjn2].x,d[adjn2].y)
  love.graphics.setColor(255,255,255)
...
function ghc(objx, objy, objx2, objy2, blockable, dir, dodgeable, force, vee, jay, dam, efftee, xjump, yjump, misc, theid)
  local bloopy = false
  hitt[1].x = me.x+5
  hitt[1].y = me.y+5
  hitt[1].j = me.j
  ...
  for i,v in ipairs(hitt) do
    ...
    then
      ...
      if not (v.dodge and dodgeable) then
        if misc == "air" then v.g = false end
...
function hboxc(P1, P2, P3, P4, blockable, dir, dodgeable, force, vee, jay, dam, flinching, efftee, xjump, yjump, misc, theid)
  hitt[1].x = me.x+5
  hitt[1].y = me.y+5
  hitt[1].j = me.j
  hitt[1].v = me.v
...
function hexplatcheck(y1, x1, x2, ex, why, w, h, v, j)
  ...
  if 
  pint(linep1, linep2, t[oppn], d[disn]) and 
  pint(linep1, linep2, t[adjn1], d[adjn2]) 
  then return true
  else return false
  end
...
function hboxp(plat)
  ...
  for i,p in ipairs(hitt) do
    if hexplatcheck(plat.y, plat.x1, plat.x2, p.x, p.y, p.width, p.height, p.v, p.j) and p.j <= 0 
